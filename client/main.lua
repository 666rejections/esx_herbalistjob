...
function OpenCloakroomMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'cloakroom',
		{
			...
			align    = 'top-left',
			elements = {
				{label = _U('herbalist_clothes_civil'), value = 'citizen_wear'},
				{label = _U('herbalist_clothes_work'), value = 'herbalist_wear'},
			},
		},
		function(data, menu)
			...
			if data.current.value == 'herbalist_wear' then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					if skin.sex == 0 then
			...
			CurrentAction     = 'herbalist_actions_menu'
...
function OpenHerbalistActionsMenu()
	...
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'herbalist_actions',
		{
			css      = 'entreprise',
			title    = _U('herbalist'),
			align    = 'top-left',
		...
		function(data, menu)
			...
			if data.current.value == 'boss_actions' then
				TriggerEvent('esx_society:openBossMenu', 'herbalist', function(data, menu)
					menu.close()
				end)
			end
...
function OpenVehicleSpawnerMenu()
	...
	if Config.EnableSocietyOwnedVehicles then
		...
		ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
			...
			ESX.UI.Menu.Open(
				...
				function(data, menu)
					...
					TriggerServerEvent('esx_society:removeVehicleFromGarage', 'herbalist', vehicleProps)
		...
		local elements = {
			{label = _U('veh_work'),  value = 'bison3'},
		}
...
function OpenMobileHerbalistActionsMenu()

	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'mobile_herbalist_actions',
		{
			css      = 'job',
			title    = _U('herbalist_actions'),
			align    = 'top-left',
			elements = {
						...
						if amount == nil or amount <= 0 then
							...
							if closestPlayer == -1 or closestDistance > 3.0 then
								...
								Citizen.CreateThread(function()
									...
									TriggerServerEvent('esx_billing:sendBill', GetPlayerServerId(closestPlayer), 'society_herbalist', 'Herbalist', amount)
								end)
							end
						end
					end,
...
function OpenGetStocksMenu()
	...
	ESX.TriggerServerCallback('esx_herbalistjob:getStockItems', function(items)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_herbalistjob:getStockItem', itemName, count)
						end
...
function OpenPutStocksMenu()
	...
	ESX.TriggerServerCallback('esx_herbalistjob:getPlayerInventory', function(inventory)
				...
				ESX.UI.Menu.Open(
					...
					function(data2, menu2)
						...
						if count == nil or count <= 0 then
							...
							TriggerServerEvent('esx_herbalistjob:putStockItems', itemName, count)
						end
...
AddEventHandler('esx_herbalistjob:hasEnteredMarker', function(zone)
	if zone == 'PlantFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist'  then
		CurrentAction     = 'plant_harvest'
		CurrentActionMsg  = _U('press_collect')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'TraitementPlant' and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist'  then
		CurrentAction     = 'plant_traitement'
		CurrentActionMsg  = _U('press_traitement')
		CurrentActionData = {zone= zone}
	end
		
	if zone == 'SellFarm' and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist'  then
		CurrentAction     = 'farm_resell'
		CurrentActionMsg  = _U('press_sell')
		CurrentActionData = {zone = zone}
	end

	if zone == 'HerbalistActions' and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
		CurrentAction     = 'herbalist_actions_menu'
		CurrentActionMsg  = _U('press_to_open')
		CurrentActionData = {}
	end
  
	if zone == 'VehicleSpawner' and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
		CurrentAction     = 'vehicle_spawner_menu'
		CurrentActionMsg  = _U('spawn_veh')
		CurrentActionData = {}
	end
		
	if zone == 'VehicleDeleter' and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
...
AddEventHandler('esx_herbalistjob:hasExitedMarker', function(zone)
	ESX.UI.Menu.CloseAll()
	if (zone == 'PlantFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
		TriggerServerEvent('esx_herbalistjob:stopHarvest')
	end  
	if (zone == 'TraitementPlant') and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
		TriggerServerEvent('esx_herbalistjob:stopTransform')
	end
	if (zone == 'SellFarm') and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
		TriggerServerEvent('esx_herbalistjob:stopSell')
	end
	CurrentAction = nil
end)
...
function blips()
	if publicBlip == false then
		local blip = AddBlipForCoord(Config.Zones.HerbalistActions.Pos.x, Config.Zones.HerbalistActions.Pos.y, Config.Zones.HerbalistActions.Pos.z)
		...
		AddTextComponentString(_U('herbalist_cloakroom'))
		EndTextCommandSetBlipName(blip)
		publicBlip = true
	end
	
    if PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
	...
	while true do
		...
		if PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' then
			...
			if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
				...
				TriggerEvent('esx_herbalistjob:hasEnteredMarker', currentZone)
			end

			if not isInMarker and HasAlreadyEnteredMarker then
				HasAlreadyEnteredMarker = false
				TriggerEvent('esx_herbalistjob:hasExitedMarker', LastZone)
			end
		end
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if CurrentAction ~= nil then
			...
			if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' and (GetGameTimer() - GUI.Time) > 300 then
				if CurrentAction == 'plant_harvest' then
					TriggerServerEvent('esx_herbalistjob:startHarvest', CurrentActionData.zone)
				end
				if CurrentAction == 'plant_traitement' then
					TriggerServerEvent('esx_herbalistjob:startTransform', CurrentActionData.zone)
				end
				if CurrentAction == 'farm_resell' then
					TriggerServerEvent('esx_herbalistjob:startSell', CurrentActionData.zone)
				end
				
				if CurrentAction == 'herbalist_actions_menu' then
					OpenHerbalistActionsMenu()
				end
				...
				if CurrentAction == 'delete_vehicle' then
					...
					if Config.EnableSocietyOwnedVehicles then
						...
						TriggerServerEvent('esx_society:putVehicleInGarage', 'herbalist', vehicleProps)
					end
		...
		if IsControlPressed(0,  Keys['F6']) and Config.EnablePlayerManagement and PlayerData.job ~= nil and PlayerData.job.name == 'herbalist' and (GetGameTimer() - GUI.Time) > 150 then
			OpenMobileHerbalistActionsMenu()
			GUI.Time = GetGameTimer()
		end
	end
end)